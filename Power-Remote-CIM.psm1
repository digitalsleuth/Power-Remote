...
#Diagnostics for timing the running of all functions against the host
    if (-not($Credential)){$Credential = Get-Credential -Message "Credentials Required"}
...
#Run all functions against the target
    ...
    if ($Credential -ne $null) {
        ...
        $Credential = Get-Credential -Message "Please Enter your Password" -UserName $Credential
        }
...
#Output to HTML file
    ...
    $get_type = ([int](Get-CimInstance win32_computersystem -CimSession $session | Select-Object -ExpandProperty PCSystemType))
    ...
    Get-CimInstance win32_computersystem -CimSession $session  |Select-Object PSComputerName,Name,Manufacturer,Domain,Model,Systemtype,PrimaryOwnerName,@{n="PC System Type";e={$pcsystemType.$get_type}},PartOfDomain,CurrentTimeZone,BootupState,@{n="Memory(Gb)";e={$_.TotalPhysicalMemory /1Gb -as [int]}} | ConvertTo-Html  -Head $htmlHeader -Body "<h5>Created on: $(Get-Date)</h5><h2>ComputerSystem</h2>" >> "$strPath"
    Get-CimInstance win32_bios -CimSession $session| Select-Object Status,Version,PrimaryBIOS,Manufacturer,@{n="Release Date";e={$_.ConvertToDateTime($_.releasedate)}},SerialNumber | ConvertTo-Html -Head $htmlHeader -Body "<h2>BIOS Information</h2>" >> "$strPath"
    Get-CimInstance win32_Useraccount -CimSession $session | Where-Object {$_.localaccount -Match 'True'} | Select-Object Name,SID,Description,Fullname,Disabled | ConvertTo-html -Head $htmlHeader -Body "<h2>Local Users</h2>" >> "$strPath"
    ((Get-CimInstance win32_groupuser -CimSession $session |Where-Object {$_.groupcomponent -like '*"Administrators"'} | ForEach-Object {$_.partcomponent -match ".+Domain\=(.+)\,Name\=(.+)$" > $nul; $matches[1].trim('"') + "\" + $matches[2].trim('"') }) -split " " | Select-Object @{n="Administrators";e={$_.Trim()}} | ConvertTo-HTML -Head $htmlHeader -Body "<h2>Administrators</h2>") -replace "\*","Administrators" >> "$strPath"
    Get-CimInstance win32_DiskDrive -CimSession $session | Select-Object Index,Model,Caption,SerialNumber,Description,MediaType,FirmwareRevision,Partitions,@{Expression={$_.Size /1Gb -as [int]};Label="Total Size(GB)"},PNPDeviceID | Sort-Object -Property Index | ConvertTo-Html -Head $htmlHeader -Body "<h2>Disk Drive Information</h1>" >> "$strPath"
    Get-CimInstance win32_share -CimSession $session | Select-Object Name,Path,Description,@{n="Type";e={$drivetype[[string]$_.Type]}}  | ConvertTo-html  -Head $htmlHeader -Body "<h2>Local Shares</h2>" >> "$strPath"
    Get-CimInstance win32_networkadapter -CimSession $session | Select-Object Name,Manufacturer,Description,AdapterType,Speed,MACAddress,NetConnectionID,PNPDeviceID | ConvertTo-Html -Head $htmlHeader -Body "<h2>Network Adapter Information</h2>" >> "$strPath"
    Get-CimInstance win32_NetworkAdapterConfiguration -CimSession $session | Select-Object @{n='IP Address';e={$_.ipaddress}},Description,@{n='MAC Address';e={$_.macaddress}},DHCPenabled,@{n="DHCPLeaseObtained";e={$_.ConvertToDateTime($_.DHCPLeaseObtained)}} | Sort-Object 'IP Address' -Descending | ConvertTo-html  -Head $htmlHeader -Body "<h2>Network Adapter Configuration</h2>" >> "$strPath"
    Get-CimInstance win32_startupCommand -CimSession $session | Select-Object Name,Location,Command,User,Caption  | ConvertTo-html  -Head $htmlHeader -Body "<h2>Startup  Software Information</h2>" >> "$strPath"
    Get-CimInstance win32_logicalDisk -CimSession $session | Select-Object DeviceID,VolumeName,@{Expression={$_.Size /1Gb -as [int]};Label="Total Size(GB)"},@{Expression={$_.Freespace / 1Gb -as [int]};Label="Free Size (GB)"},FileSystem, VolumeSerialNumber |  ConvertTo-html  -Head $htmlHeader -Body "<h2>Disk Information</h2>" >> "$strPath"
    Get-CimInstance win32_operatingsystem -CimSession $session | Select-Object Caption,OSArchitecture,Organization,$InstallDate,Version,SerialNumber,BootDevice,WindowsDirectory,CountryCode,@{n="Last Bootup";e={$_.ConvertToDateTime($_.lastbootup)}},@{n="Local Date/Time";e={$_.ConvertToDateTime($_.LocalDateTime)}} | ConvertTo-html  -Head $htmlHeader -Body "<h2>OS Information</h2>" >> "$strPath"
...
#Artifacts then will be copied from the target to the destination, then deleted from the target
    ...
    foreach($key in $artifacts.Keys){
        Invoke-CimMethod Win32_process -MethodName Create -Arguments @{CommandLine = "$shell" + $artifacts.$key} -CimSession $session -ErrorAction stop | Out-Null
    ...
    foreach($file in $fileList){
        ...
        Write-ProgressHelper -StatusMessage "Removing $file from host" -StepNumber ($stepCounter++)
        Start-sleep -s 1
...
#Can be configured to retrieve the wireless key using the key=clear command, but is not enabled by default
    ...
    Invoke-CimMethod win32_process -MethodName Create -Arguments @{CommandLine = "$shell$wireless"} -CimSession $session -ErrorAction stop | Out-Null
    Start-Sleep -s 10
    $drivemount = (Get-ChildItem function:[d-z]: -n | Where-Object { !(test-path $_) } | Select-Object -First 1) -replace ":",""
    New-PSDrive -Name $drivemount -PSProvider filesystem -Root $net_path @credsplat | Out-Null
...
$AppCompatCache = (Invoke-CimMethod -Namespace "root\default" -ClassName "StdRegProv" -MethodName "GetBinaryValue" -Arguments @{hDefKey = [uint32]2147483650; sSubKeyName = $shimcachelocation; sValueName = "AppCompatCache"} -CimSession $session) | Select -expand uValue
...
#Copy the winpmem exec to the remote host, create a memory dump, copy to the originating source, and delete the results from the target
    ...
    Try {
        ...
        $invokeMemDump = (Invoke-CimMethod win32_process -MethodName Create -Arguments @{CommandLine = $net_path + "\winpmem.exe --format raw -o " + $driveLetter + "memory.raw"} -CimSession $session -ErrorAction stop)
        $memdumpPID = $invokeMemDump.processID
        $memdumpRunning = { Get-CimInstance -Class win32_process -Filter "ProcessID='$memdumpPID'" -CimSession $session -ErrorAction SilentlyContinue | Where-Object { ($_.ProcessName -eq 'winpmem.exe') } }
...
#Gather USB Drive Letters, Serials, Usernames who inserted devices, First and Last Insert etc.
    ...
    Invoke-CimMethod win32_process -MethodName Create -Arguments @{CommandLine = $powershell + '$Volumes = @(); Get-Item HKLM:\System\MountedDevices | Select-Object -ExpandProperty Property | where {$_ -like \"\??\Volume*\"} | ForEach-OBject {$volume = $_; $Volumes += New-Object -TypeName psobject -Property @{ Volume = $volume -replace \"\\\?\?\\Volume\",\"\"; KeyValue = ((Get-ItemProperty HKLM:\System\MountedDevices -Name $Volume).\"$volume\" | ForEach-Object{[convert]::ToString($_, 16)}) -join \"\" ; ASCII = ((Get-ItemProperty HKLM:\System\MountedDevices -Name $Volume).\"$volume\" | ForEach-Object{[convert]::ToChar($_)}) -join \"\" -replace \"\x00\",\"\" }}; $Volumes | Select-Object Volume,ASCII,KeyValue | Export-CSV -NoTypeInformation \"$driveletter\volumes.csv\"'} -CimSession $session -ErrorAction Stop | Out-Null
...
param($ComputerName,[ValidateNotNullOrEmpty()]$Credential,[switch]$HKCR,[switch]$HKCU,[switch]$HKLM,[switch]$HKU,[switch]$HKCC,$Subkey,$Value)
    ...
    $RegProvider = Invoke-CimMethod -ClassName "StdRegProv" -Namespace "root\default" -MethodName EnumValues -Arguments @{hDefKey = [uint32]$hive; sSubKeyName = $subkey} -CimSession $session
    $sNames = $RegProvider | Select -expand sNames
    $Types = $RegProvider | Select -expand Types
    ...
    switch ($Type) {
          ...
          {
            $KeyType = 'REG_SZ'
            $ReturnKeyValue = (Invoke-CimMethod -ClassName "StdRegProv" -Namespace "root\default" -MethodName GetStringValue -Arguments @{hDefKey = [uint32]$hive; sSubKeyName =  $subkey; sValueName = $value} -CimSession $session).sValue
            Break
          }
          $REG_EXPAND_SZ
          {
            $KeyType = 'REG_EXPAND_SZ'
            $ReturnKeyValue = (Invoke-CimMethod -ClassName "StdRegProv" -Namespace "root\default" -MethodName GetExpandedStringValue -Arguments @{hDefKey = [uint32]$hive; sSubKeyName =  $subkey; sValueName = $value} -CimSession $session).sValue
            Break
          }
          $REG_BINARY
          {
            $KeyType = 'REG_BINARY'
            $ReturnKeyValue =  (Invoke-CimMethod -ClassName "StdRegProv" -Namespace "root\default" -MethodName GetBinaryValue -Arguments @{hDefKey = [uint32]$hive; sSubKeyName =  $subkey; sValueName = $value} -CimSession $session).uValue
            Break
          }
          $REG_DWORD
          {
            $KeyType = 'REG_DWORD'
            $ReturnKeyValue = (Invoke-CimMethod -ClassName "StdRegProv" -Namespace "root\default" -MethodName GetDWORDValue -Arguments @{hDefKey = [uint32]$hive; sSubKeyName =  $subkey; sValueName = $value} -CimSession $session).uValue
            Break
          }
          $REG_MULTI_SZ
          {
            $KeyType = 'REG_MULTI_SZ'
            $ReturnKeyValue = (Invoke-CimMethod -ClassName "StdRegProv" -Namespace "root\default" -MethodName GetMultiStringValue -Arguments @{hDefKey = [uint32]$hive; sSubKeyName =  $subkey; sValueName = $value} -CimSession $session).sValue
            Break
          }
          $REG_QWORD
          {
            $KeyType = 'REG_QWORD'
            $ReturnKeyValue = (Invoke-CimMethod -ClassName "StdRegProv" -Namespace "root\default" -MethodName GetQWORDValue -Arguments @{hDefKey = [uint32]$hive; sSubKeyName =  $subkey; sValueName = $value} -CimSession $session).sValue
            Break
          }
        }
...
param($ComputerName,[ValidateNotNullOrEmpty()]$Credential,[Parameter(Mandatory=$True)][int]$Timespan)
    ...
    Invoke-CimMethod win32_process -MethodName Create -Arguments @{CommandLine=$netstart + $driveLetter + $filename} -CimSession $session -ErrorAction SilentlyContinue | Out-Null
